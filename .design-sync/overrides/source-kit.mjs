...
// forked from design-sync lib/source-kit.mjs — also drop componentSrcMap-null
// components from the SYNTH ENTRY (not just the component list), so their heavy
// WebGL/map deps (maplibre-gl, leaflet, three) don't blow the 5MB bundle cap.
...
export async function resolvePackage(ctx) {
  ...
  // FORK: files whose component is excluded via componentSrcMap:null are dropped
  // from BOTH the synth entry (so their heavy transitive deps never enter the
  // bundle) AND from src-based discovery below (so their other PascalCase
  // exports — e.g. map.tsx's MapMarker/MapArc — aren't listed as components the
  // bundle doesn't actually export → [BUNDLE_EXPORT]). Match each file's
  // basename against every null key in both PascalCase and kebab-case form.
  const excludedBases = new Set(
    Object.entries(srcMap)
      .filter(([, v]) => v === null)
      .flatMap(([name]) => [name.toLowerCase(), name.replace(/([a-z0-9])([A-Z])/g, '$1-$2').toLowerCase()]),
  );
  const entryImplFiles = srcFiles.filter((p) => {
    if (!SRC_IMPL_RX.test(p) || NON_IMPL_RX.test(p)) return false;
    const base = basename(p).replace(/\.(tsx|jsx)$/, '').toLowerCase();
    return !excludedBases.has(base);
  });
  ...
  if (!entry) {
    ...
    writeFileSync(entry, entryImplFiles.map((p) => `export * from ${JSON.stringify(p)};`).join('\n') + '\n');
    synthEntry = true;
    console.error(
      `[NO_DIST] no built entry — synthesizing from ${entryImplFiles.length} src files (run the package's build for best results)`,
    );
  }
  ...
  if (!components.length && synthEntry) {
    // FORK: discover from the SAME filtered file set the entry was built from,
    // so excluded files don't contribute components the bundle won't export.
    components = deriveComponentsFromSrc(entryImplFiles).filter((c) => srcMap[c.name] !== null);
  }
